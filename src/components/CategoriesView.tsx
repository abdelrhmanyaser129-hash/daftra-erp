...
import { ArrowLeft, Plus, Edit3, Trash2, Check, X, Tag } from 'lucide-react';
...
export default function CategoriesView({ setView }: CategoriesViewProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (error) { alert('فشل تحديث التصنيف: ' + error.message); return; }
    } else {
      const { error } = await supabase.from('product_categories').insert(payload);
      if (error) { alert('فشل إنشاء التصنيف: ' + error.message); return; }
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) { alert('فشل حذف التصنيف: ' + error.message); return; }
    load();
  };
    ...
    <div className="max-w-3xl mx-auto space-y-4 text-right font-sans select-none">
      <div className="flex items-center justify-between bg-white p-3 rounded-md border border-[#d4e1ed]">
        <div className="flex items-center gap-2 flex-row-reverse">
          ...
          <h2 className="text-lg font-bold text-slate-800">إدارة التصنيفات</h2>
        </div>
        <button onClick={handleOpenAdd}
          ...
          <Plus className="w-3.5 h-3.5" /><span>إضافة تصنيف</span>
        </button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-base font-bold text-slate-800 mb-4">{editingId ? 'تعديل التصنيف' : 'إضافة تصنيف جديد'}</h3>
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">اسم التصنيف <span className="text-rose-500">*</span></label>
      ...
      <div className="bg-white rounded-md border border-[#d4e1ed] overflow-hidden">
        <div className="bg-[#fcfdfe] px-4 py-2.5 border-b border-[#e9eff4]">
          <span className="text-slate-800 font-bold block">قائمة التصنيفات</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {items.length === 0 && <tr><td colSpan={3} className="text-center py-8 text-slate-400">لا توجد تصنيفات مضافة بعد</td></tr>}
              {items.map(item => (
